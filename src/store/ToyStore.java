...
public class ToyStore {
    ...
    public static void main(String[] args) {
        if (args.length == 2){
            AlsToyBarn toyBarn = new AlsToyBarn(Integer.parseInt(args[0]), Integer.parseInt(args[1]));
            while(toyBarn.availableStock() > 0) {
                IToy toy = toyBarn.purchaseToy();
                System.out.println("The next customer purchases " +toy.toString() + " and begins to play with it...");
                toy.play();
                toyBarn.addToy(toy);
                System.out.println("The customer grows bored with the toy and trades it in for " +
                        String.format("$%.2f", toy.getResaleValue()));
            }
            System.out.print("The toy store is out of stock! Time to close!");
        }
    }
}
